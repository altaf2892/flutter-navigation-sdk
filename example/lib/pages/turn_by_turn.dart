...
class TurnByTurnPage extends ExamplePage {
  const TurnByTurnPage({super.key}) : super(leading: const Icon(Icons.roundabout_right), title: 'Turn-by-turn');
...
class _TurnByTurnPageState extends ExamplePageState<TurnByTurnPage> {
  ...
  Future<void> _startNavigation() async {
    ...
    if (!await GoogleMapsNavigator.areTermsAccepted()) {
      await GoogleMapsNavigator.showTermsAndConditionsDialog('test_title', 'test_company_name');
    }
    ...
    /// Simulate location.
    await GoogleMapsNavigator.simulator.setUserLocation(const LatLng(latitude: 37.528560, longitude: -122.361996));

    final Destinations msg = Destinations(
      waypoints: <NavigationWaypoint>[
        NavigationWaypoint.withLatLngTarget(title: 'Grace Cathedral', target: const LatLng(latitude: 37.791957, longitude: -122.412529)),
      ],
  ...
  Future<void> _setupListeners() async {
    ...
    // to preview.
    _navInfoSubscription = GoogleMapsNavigator.setNavInfoListener(_onNavInfoEvent, numNextStepsToPreview: 100);
  }
    ...
    (BuildContext context) => SafeArea(
      child: Column(
        children: <Widget>[
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                onPressed:
                    _navigationRunning
                        ? () async {
                          await _navigationViewController.setNavigationHeaderEnabled(!(await _navigationViewController.isNavigationHeaderEnabled()));
                          await _navigationViewController.setNavigationFooterEnabled(!(await _navigationViewController.isNavigationFooterEnabled()));
                        }
                        : null,
          ...
          if (_navInfo != null) ElevatedButton(onPressed: () => _showSteps(context, _navInfo!), child: const Text('Show all remaining steps')),
        ],
      ),
    ),
  );
  ...
  Widget _getNavInfoWidgets(BuildContext context, NavInfo navInfo) {
    const TextStyle navInfoTextStyle = TextStyle(fontSize: 12, color: Colors.white);
    return Container(
      padding: const EdgeInsets.all(20),
      child: Column(
        children: <Widget>[
          if (navInfo.navState == NavState.stopped) const Text('Navigation stopped.'),
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    Column(
                      children: <Widget>[
                        if (navInfo.timeToFinalDestinationSeconds != null && navInfo.distanceToFinalDestinationMeters != null)
                          Text(
  ...
  /// Look at the [NavInfo] documentation for more information.
  Widget _getNavInfoWidgetForStep(BuildContext context, StepInfo stepInfo, int? metersToStep) {
            ...
            Expanded(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Text('Step #${stepInfo.stepNumber}', style: navInfoTextStyle),
                    const SizedBox(height: 10),
                    RichText(
                      text: TextSpan(
                        style: navInfoTextStyle,
                        children: <TextSpan>[
                          const TextSpan(text: 'Road: ', style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            SizedBox(
              ...
              child: Scrollbar(
                child: SingleChildScrollView(
                  child: Column(
                    children: <Widget>[
                      if (navInfo.remainingSteps.isEmpty) const Text('No remaining steps'),
                      for (final StepInfo step in navInfo.remainingSteps) _getNavInfoWidgetForStep(context, step, null),
